...
--*/
{
    ...
    //
    WdfFdoInitSetFilter(DeviceInit);//告诉frame这是一个filter驱动
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        WdfRequestComplete(Request, status);//不成功也要完成，不往下面走了？
        return;
    }
...
#else   
        FilterForwardRequest(Request, WdfDeviceGetIoTarget(device));//要点，学习！
#endif
...
--*/
{
    ...
    ret = WdfRequestSend(Request, Target, &options);//fire！
